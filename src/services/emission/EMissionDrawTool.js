...
/**
 * EMissionDrawTool — click-to-add-waypoint E-Mission placement.
 *
 * States: idle → placing → placed
 *
 * Flow:
 *   1. activate()      — crosshair cursor, opens EMissionPopup in new mode
 *   2. First click     → addMission() to EMissionManager, popup attaches to mission
 ...
 */
export class EMissionDrawTool {
  constructor({ map, eMissionManager, eMissionPopup }) {
    this._map = map;
    this._manager = eMissionManager;
    this._popup = eMissionPopup; // may be null initially — set via late binding

    this._state = 'idle';         // 'idle' | 'placing' | 'placed'
    this._placingEnabled = true;
    this._activeMissionId = null;
    this._previewLine = null;     // dashed red preview line: last WP → cursor
  ...
  /** Called by EMissionPopup Done. Cleans up preview and returns to idle. */
  resetToIdle() {
    this._cleanupPreview();
    this._map.off('click', this._onNextClick);
    this._map.off('mousemove', this._onMouseMove);
  ...
  _onNextClick(e) {
    ...
    const rec = this._manager.missions.find((m) => m.id === this._activeMissionId);
    if (!rec) return;
    // Block adding WPs after Last 6 is placed
    if (rec.waypoints.some((wp) => wp.lastSix)) return;
  ...
  _startInsertPreview(rec, afterIndex, fromLatLng) {
    ...
    this._previewLine = L.polyline([ll, ll], {
      color: '#e63946', weight: 1.5, dashArray: '5,5',
