/**
 * EMissionPanel — side-panel pushed from PlanPanel.
 * Shows DRAW E-MISSION button + table of saved E-missions.
 */
export class EMissionPanel {
  constructor({ sideMenu, eMissionManager, eMissionTool, eMissionPopup }) {
    this._sideMenu = sideMenu;    // may be null — set via late binding
    this._manager = eMissionManager;
    this._eMissionTool = eMissionTool;
    this._popup = eMissionPopup;
  ...
  _build() {
    ...
    drawBtn.textContent = 'Draw E-Mission';
  ...
  _refreshTable() {
    ...
    if (missions.length === 0) {
      ...
      empty.textContent = 'No E-missions yet.';
  ...
  _makeRow(mission) {
    ...
    const hasLast6 = mission.waypoints?.some((wp) => wp.lastSix);
    infoTd.textContent = `${wpCount} WP${hasLast6 ? ' ⬡' : ''}`;
